...
uint64_t micros_huge(void) {
	...
	uint8_t oldSREG = SREG; // Save and restore the interrupt enable bit
	cli();
	uint64_t result = ((uint64_t)seconds_count)*1000000UL + TCD1.CNT*1000UL + (TCD0.CNT>>2);
	SREG = oldSREG;
...
unsigned long micros(void) {
	// disable interrupts while we read millis_cnt or we might get an
	// inconsistent value (e.g. in the middle of a write to millis_cnt)

        // TODO: Will micros and millis be consistent?
        // Are events processed even when interrupts are disabled?
        // If so TCD1 count may be updated even when interrupts are off.
        uint16_t millis_counter_value;
        uint16_t micros_counter_value;
	unsigned long current_seconds_count;
	unsigned long current_millis_count;
	
        

	uint8_t oldSREG = SREG; // Save and restore the interrupt enable bit
	cli();
	current_millis_count = millis_count;
        current_seconds_count = seconds_count;
	micros_counter_value = TCD0.CNT;
	millis_counter_value = TCD1.CNT;
	SREG = oldSREG;

	return current_seconds_count*1000000UL + current_millis_count*1000 + millis_counter_value*1000UL + (micros_counter_value>>2);
}
...
void init()
{
	...
	*/
	...
	// Use 2MHz internal oscillator and scale it to 32MHz with PLL
	...
	// Enable DFLL to improve accuracty of oscillator
	uint16_t DFLLRatio = (F_CPU / 1024);
	DFLLRC2M.COMP1  = (DFLLRatio & 0xFF);
	DFLLRC2M.COMP2  = (DFLLRatio >> 8);
